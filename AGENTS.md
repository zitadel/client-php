...
- Generator: `php-plus`, run from the `openapi-generator-plus:enhanced` image.
...
Hand-written here: the bespoke authenticators under `lib/Auth/`, their tests, the
...
devbox run -- make test       # composer test
...
the trace-context tests). This repo keeps its own `composer.json`, so the generator cannot add
